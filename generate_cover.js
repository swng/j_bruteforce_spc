...
function color_the_greyed_J(fumens) { // array of fumens
    let results = [];
    ...
    (async () => {
        for await (const line of rl) {
            ...
            if (lineCount <= 50) {
                ...
                fumens = color_the_greyed_J(fumens);
                fumens = [...new Set(fumens)];
                glued_fumens = glue(fumens);

                all_fumens.push(...glued_fumens);

                let name = `J-s-${String(lineCount).padStart(2, '0')}`;
        ...
        let command = `java -jar sfinder.jar cover -K +t -d 180 -p 'J,*!' -t '${all_fumens.join(' ')}' --mode tss`
        console.log(command);
